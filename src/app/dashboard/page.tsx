...
export default function DashboardPage() {
      ...
      try {
        ...
        if (casesRes.ok) {
          try {
            ...
            const trendRes = await fetch("/api/scales/trend", {
              cache: "no-store",
            });
            if (trendRes.ok) {
  ...
  const trendMax = trendData.reduce(
        ...
        <div className="mx-auto flex max-w-5xl flex-col gap-6 px-4 pb-24 pt-4 md:pb-12">
                    ...
                    {(() => {
                      ...
                      const short =
                        typeof ev.short_sentence === "object"
                          ? (ev.short_sentence?.zh ??
                            ev.short_sentence?.en ??
                            "")
            ...
            {recentEvents.length === 0 ? (
              <>
                <p className="text-lg text-neutral-200 mt-4">
                  每一段經驗、每一次觀察，都可以慢慢被放在這裡，不需要一次完成。
                </p>
          ...
          <section className="space-y-3 rounded-2xl border border-neutral-800 bg-neutral-900/70 p-5 shadow-sm">
            ...
            {trendData.length === 0 ? (
              ...
              <div className="space-y-2">
                {trendData.map((item) => {
                  const barWidth = trendMax
                    ? Math.max((item.count / trendMax) * 100, 5)
                    : 0;
                  return (
                    <div
                      key={item.label}
          ...
          <section className="space-y-3 rounded-2xl border border-neutral-800 bg-neutral-900/70 p-5 shadow-sm">
            ...
            <div className="space-y-2 text-sm text-neutral-200">
              <div>
                最近 MMSE：
                {latestScales.find((s) => s.scale_type === "MMSE")
                  ?.total_score ?? "—"}
              </div>
              <div>
                最近 CDR：
                {latestScales.find((s) => s.scale_type === "CDR")
                  ?.total_score ?? "—"}
              </div>
            </div>
          </section>
